...
//-----------------------Begin Bug Fix: http://www.garagegames.com/mg/forums/result.thread.php?qt=23223
   mRTSUnitTypeName = StringTable->insert("");
//-----------------------End Bug Fix 
   mBaseDamage = 10;
   mAttackDelay = 320;
   mDamagePlus = 3;
...
//-----------------------Begin Bug Fix: http://www.garagegames.com/mg/forums/result.thread.php?qt=23223
   stream->writeString(mRTSUnitTypeName);
//-----------------------End Bug Fix 
...
//-----------------------Begin Bug Fix: http://www.garagegames.com/mg/forums/result.thread.php?qt=23223
   mRTSUnitTypeName = stream->readSTString();
//-----------------------End Bug Fix 
...
//-----------------------Begin Bug Fix: http://www.garagegames.com/mg/forums/result.thread.php?qt=23223
   addField("RTSUnitTypeName",		TypeString, Offset(mRTSUnitTypeName,	RTSUnitData));
//-----------------------End Bug Fix 
...
//-----------------------Begin Bug Fix: http://www.garagegames.com/mg/forums/result.thread.php?qt=23701
...
/*      else
      ...
      } */ // JY - disable my "hack" to force footprints, checking if the DSQ changes fixed it
      ...
      for (int i = 0; i < MaxSoundThreads; i++)
      {
           ...
           if (stream->writeFlag(mask & (SoundMaskN << i)))
               if (stream->writeFlag(st.play))
                    stream->writeRangedU32(st.profile->getId(),DataBlockObjectIdFirst, DataBlockObjectIdLast);
      }
   }
      ...
      for (S32 i = 0; i < MaxSoundThreads; i++)
      {
           if (stream->readFlag())
           {
                ...
                if ((st.play = stream->readFlag()) == true)
                     st.profile = (AudioProfile*) stream->readRangedU32(DataBlockObjectIdFirst,DataBlockObjectIdLast);
                if (isProperlyAdded())
                     updateAudioState(st);
           }
      }
...
//-----------------------Begin Bug Fix: http://www.garagegames.com/mg/forums/result.thread.php?qt=23223
ConsoleMethod( RTSUnit, getRTSUnitTypeName, const char *, 2, 2, "()"
              "Returns a text string of the RTSUnit's type name (defined in RTSUnitData).")
{
   char *returnBuffer = Con::getReturnBuffer( 256 );
   dSprintf( returnBuffer, 256, "%s", object->getUnitTypeName() );
