...
//-----------------------------------------------------------------------------
// bug fix added Sept 22nd, 2007 from Andy Maloney's forum post *28015
...
//-----------------------Begin Bug Fix: http://garagegames.com/mg/forums/result.thread.php?qt=28015
u16*            fluid::m_pIndex     = NULL;
u16*            fluid::m_pINext     = NULL;
s32             fluid::m_IOffset    = 0;
//-----------------------End Bug Fix
...
fluid::vertex* fluid::AcquireVertices( s32 Count )
{
    ...
    m_IOffset = m_VUsed;
//-----------------------Begin Bug Fix: http://garagegames.com/mg/forums/result.thread.php?qt=28015
    AssertFatal( (m_IOffset >= 0 ) && (m_IOffset <= S32_MAX), "fluid::AcquireVertices: Invalid m_IOffset" );	
//-----------------------End Bug Fix
	m_VUsed  += Count;
...
void fluid::AddTriangleIndices( s16 I1, s16 I2, s16 I3 )
{
    if( m_IAllocated == 0 )
    {
        ...
        m_IUsed      =   0;
//-----------------------Begin Bug Fix: http://garagegames.com/mg/forums/result.thread.php?qt=28015
        m_pIndex     = (u16*)MALLOC( m_IAllocated * sizeof(s16) );
//-----------------------End Bug Fix
        m_pINext     = m_pIndex;
        ASSERT( m_pIndex );
    }
    ...
    if( m_IUsed+3 > m_IAllocated )
    {
        ...
        m_IAllocated += 100;
//-----------------------Begin Bug Fix: http://garagegames.com/mg/forums/result.thread.php?qt=28015
        m_pIndex      = (u16*)REALLOC( m_pIndex, m_IAllocated * sizeof(s16) );
//-----------------------End Bug Fix
