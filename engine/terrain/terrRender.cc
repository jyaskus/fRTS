...
void TerrainRender::emitTerrChunk(SquareStackNode *n, F32 squareDistance, U32 lightMask, SelectionField selectionMask, bool farClip, bool drawDetails, bool drawBumps)
{
   //if(n->pos.x || n->pos.y)
   //   return;
...
// CW - stuff with bump maps
            if (mEnableTerrainEmbossBumps && squareDistance < zeroBumpDistance)
                drawBumps = true;
// CW - end bump map stuff
         emitTerrChunk(n, squareDistance, n->lightMask, n->selectionMask, nextClipFlags & FarSphereMask, drawDetails, drawBumps);
         curStackSize--;
         continue;
      }
      ...
      for (U32 i = 0; i < MaxTerrainSelections; i++) {
         ...
         for (U32 start = startXFIndex; start < mXFIndexCount; start++) {
            if (mXFIndexBuffer[start] == GL_TRIANGLE_FAN) {
               ...
               LightTriangle* selectionTris = (LightTriangle*)FrameAllocator::alloc(sizeof(LightTriangle) * triCount);
               U32 j;
               for (j = 0; j < (triCount-1); j++)
