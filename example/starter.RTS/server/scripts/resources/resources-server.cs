...
// echo ("resourceStore::Ctor()");    
  ...
  {  
  	...
  	dangerousLow = 50;        // Resource is running very low...  
  	gettingLow = 100;         // running low..  
...
//		    "supply is" SPC %mSupply.getName() SPC "Total store count is" SPC %store.getCount());
		switch$(%supplyIndex) // each of our 3 resources start off with a diff amount
		{
		  case "0": // gold
		   %mSupply.count = 500;
		   %supplyInventory = "Gold" SPC "500";
	  	case "1": // wood
	  	   %mSupply.count = 250;
		   %supplyInventory = %supplyInventory SPC "Wood" SPC "250";
	  	case "2": // food
		   %mSupply.count = 500;
		   %supplyInventory = %supplyInventory SPC "Food" SPC "500";
		  case "3": // stone
		   %mSupply.count = 250;
		   %supplyInventory = %supplyInventory SPC "Stone" SPC "250";
		}
	}
