...
addMessageCallback( 'MsgTeamInfo', handleTeamInfoMessage );

function handleTeamInfoMessage(%msgType, %msgString, %info)
{
   $Server::maxTeams = getField(%info, 0);
   for (%i = 0; %i < $Server::maxTeams; %i++)
      $Server::TeamInfo[%i] = getField(%info, %i+1);
}
...
function RTSConnection::initialControlSet(%this)
{
   ...
   if (%this.getControlObject().getClassName() $= "RTSCamera")
   {
      $RTSCamera = %this.getControlObject();
      
      // Just some defaults
      $RTSCamera.setOrbitDistance( 50 );
      
      if( $RunningDemo )
      {
         $RTSCamera.setCameraPosition( -16, -14 );
      }
      else
      {
         $RTSCamera.setPitchAngle( 60 );
         $RTSCamera.setCameraPosition( -40, -57 );
      }
      
   }
}

function RTSConnection::onConnectionAccepted(%this)
{
   Canvas.setContent(LobbyGui);
   LobbyChatCtrl.attach(HudMessageVector);
}

function RTSConnection::onConnectionTimedOut(%this)
{
...
function RTSConnection::onConnectionDropped(%this, %msg)
{
...
function RTSConnection::onConnectionError(%this, %msg)
{
...
function RTSConnection::onConnectRequestRejected( %this, %msg )
{
   switch$(%msg)
   {
      case "CR_INMISSION":
      ...
      case "CHR_PROTOCOL":
         %error = "Incompatible protocol version: Your game version is not compatible with this server.";
      case "CHR_INMISSION":
         %error = "You can only join servers when they are in the lobby.";
      case "CHR_CLASSCRC":
...
function RTSConnection::onConnectRequestTimedOut(%this)
{
   disconnectedCleanup();
...
function disconnect()
{
   ...
   // JY - moving it here, to avoid crashing the game when game over   
